...
static int getkeystring(INI_FILETYPE *fp, const TCHAR *Section,
  ...
  if (len > 0 || idxSection >= 0) {
    ...
    } while ((((int)(ep - sp) != len || Section == NULL ||
               _tcsnicmp(sp, Section, len) != 0) &&
              ++idx != idxSection));
    if (idxSection >= 0) {
      if (idx == idxSection) {
        assert(ep != NULL);
...
int ini_hassection(const mTCHAR *Section, const mTCHAR *Filename) {
  ...
  if (ini_openread(Filename, &fp)) {
    ok = getkeystring(&fp, Section, NULL, -1, 0, LocalBuffer,
                      sizearray(LocalBuffer), NULL);
    (void)ini_close(&fp);
  }
  return ok;
}
 ...
 */
int ini_haskey(const mTCHAR *Section, const mTCHAR *Key,
  ...
  if (ini_openread(Filename, &fp)) {
    ok = getkeystring(&fp, Section, Key, -1, -1, LocalBuffer,
                      sizearray(LocalBuffer), NULL);
    (void)ini_close(&fp);
  }
  return ok;
